...
function createArtisticTree() {
  ...
  const garlandSpirals = 7;
  const weaveAmount = 15;
  for (let y = topY; y < bottomY; y += 3) {
    let progress = map(y, topY, bottomY, 0, 1);

    let baseRadius =
      progress * maxRadius * map(noise(progress * 5), 0, 1, 0.8, 1.2);
    ...
    for (let i = 0; i < particlesToSpray; i++) {
      ...
      let tinselColor =
        random(1) > 0.5 ? color(255, 215, 0, 200) : color(192, 192, 192, 200);
      treeParticles.push({
        pos: createVector(particleX, particleY),
        color: tinselColor,
      });
  ...
  for (let i = 0; i < 100; i++) {
    ...
    treeParticles.push({
      pos: createVector(
        centerX + random(-trunkWidth, trunkWidth),
        trunkY + random(trunkHeight)
      ),
      color: brownShade,
    });
  }
...
function drawSnowDrift() {
  ...
  for (let x = 0; x <= designWidth; x += 10) {
    let y =
      designHeight -
      snowDriftHeight +
      noise(x * 0.01 + frameCount * 0.005) * 40;
    vertex(x, y);
  }
